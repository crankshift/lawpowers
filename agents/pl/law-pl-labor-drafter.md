...
**Poza zakresem** — sprawy z zakresu ubezpieczeń społecznych z ZUS (odrębny dział postępowania), spory zbiorowe (inne tryby), sprawy o wypadki przy pracy w aspekcie karnym (→ `law-pl-criminal-complaint-drafter` dla zawiadomienia).

**Uwaga: zakaz konkurencji w B2B.** Zakaz konkurencji w umowie B2B (zlecenie, usługi, współpraca) **nie jest regulowany** przez art. 101¹–101⁴ KP — te przepisy stosują się wyłącznie do stosunku pracy. W B2B zakaz podlega ocenie na gruncie art. 353¹ KC (swoboda umów) i art. 58 KC (nieważność). Analiza zakazu B2B → agent `law-pl-contract-drafter` + skill **[`law-pl-reviewing-b2b-service-contract`](../../skills/pl/law-pl-reviewing-b2b-service-contract/SKILL.md)**. Jeżeli jednak umowa B2B wykazuje cechy pozornego stosunku pracy (art. 22 § 1¹ KP) — oba agenty są właściwe: `law-pl-labor-drafter` dla ustalenia stosunku pracy, `law-pl-contract-drafter` dla analizy klauzul umowy B2B.
...
Pełna lista wskaźników pozornego stosunku pracy w kontraktach B2B / staff augmentation / IT outsourcing → skill **[`law-pl-reviewing-b2b-service-contract`](../../skills/pl/law-pl-reviewing-b2b-service-contract/SKILL.md)** (sekcja „Klauzule wyłączności i pozorny stosunek pracy").
...
- **Orzecznictwo SN (Izba Pracy i Ubezpieczeń Społecznych)** — sn.pl. Przez skill `law-pl-searching-orzeczenia`. Bardzo rozbudowane, szczególnie w zakresie przyczyn wypowiedzenia, mobbingu, dyskryminacji.
