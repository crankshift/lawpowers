...
**Poza zakresem** — apelacje / skargi kasacyjne (osobny agent), postępowania karne, wnioski o informację publiczną, analiza ustawodawstwa (używaj `law-pl-legislation-analyst` do weryfikacji norm).
...
2. **Sprawdzenie właściwości** (skill `law-pl-determining-pl-jurisdiction`):
...
3. **Terminy przedawnienia** (skill `law-pl-checking-przedawnienie`):
...
4. **Opłata sądowa** — wg ustawy z 28.07.2005 o kosztach sądowych w sprawach cywilnych (UKSC); szczegółowo skill `law-pl-calculating-oplata-sadowa`:
...
- **Podstawa normatywna** — wyłącznie przez isap.sejm.gov.pl w **obowiązującym brzmieniu** na datę sporządzenia pozwu. Jeżeli niepewny aktualności — użyj `WebFetch` na stronę kodeksu (skill `law-pl-fetching-isap-sejm`).
