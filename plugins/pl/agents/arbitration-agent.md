...
description: Postępowanie arbitrażowe — międzynarodowy arbitraż handlowy (ICC, LCIA, SCC, SIAC, HKIAC, VIAC, ad hoc UNCITRAL), krajowe sądy polubowne (SAKIG przy KIG, Sąd Arbitrażowy Lewiatan, AT przy SIDiR), arbitraż inwestycyjny (ICSID, UNCITRAL pod BIT/ECT). Sporządzanie Request for Arbitration i pozwów do sądu polubownego, Statement of Defence, powództw wzajemnych, wniosków w toku arbitrażu; nietypowych klauzul arbitrażowych i audyt na patologiczność; nominacja / wyłączenie arbitra wg IBA Guidelines; wniosków o uznanie i stwierdzenie wykonalności zagranicznego wyroku arbitrażowego w Polsce wg art. 1212–1217 KPC i Konwencji Nowojorskiej 1958; skarg o uchylenie wyroku sądu polubownego (art. 1205–1211 KPC); zabezpieczeń (interim, emergency arbitrator) równolegle z arbitrażem. NIE wywoływać dla standardowych klauzul arbitrażowych w zwykłych umowach handlowych (→ contract-drafter), dla postępowania egzekucyjnego po postanowieniu sądu powszechnego o stwierdzeniu wykonalności (→ enforcement-agent), dla opinii prawnych ogólnych, w których arbitraż jest elementem ubocznym (→ legal-memo), dla apelacji/kasacji w sprawach NIE-arbitrażowych (→ appeal-drafter).
