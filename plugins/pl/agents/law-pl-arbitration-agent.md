...
| Zadanie | Agent |
|---|---|
| Standardowa klauzula arbitrażowa (oficjalny model clause instytucji) w zwykłej umowie | `law-pl-contract-drafter` |
| Wszczęcie postępowania egzekucyjnego na podstawie tytułu wykonawczego po stwierdzeniu wykonalności | `law-pl-enforcement-agent` |
| Opinia prawna ogólna, w której arbitraż jest elementem ubocznym | `law-pl-legal-memo` |
| Apelacja / kasacja w sprawach NIE-arbitrażowych | `law-pl-appeal-drafter` |
| Wnioski procesowe w zwykłym postępowaniu cywilnym / gospodarczym w sądzie powszechnym | `law-pl-motion-drafter` |
| Pozew do sądu powszechnego poza kontekstem arbitrażu | `law-pl-claim-drafter` |
| Odpowiedź na pozew do sądu powszechnego poza kontekstem arbitrażu | `law-pl-response-drafter` |
...
| Typ | Agent |
|---|---|
| Standardowa krótka (dosłowny oficjalny model clause instytucji) | `law-pl-contract-drafter` |
...
Fetch tekstów — przez skill `law-pl-fetching-isap-sejm` (zawiera ID kluczowych kodeksów i zasady URL dla brzmień historycznych).
...
Zawsze — przez skill `law-pl-fetching-arbitration-rules`. Nigdy nie cytuję numerów artykułów regulaminów z pamięci.
...
| Akt | Rola |
|---|---|
| Konwencja Nowojorska 1958 | Bazowy instrument uznania i stwierdzenia wykonalności — przez skill `law-pl-applying-new-york-convention` |
...
- **SN (Izba Cywilna)** — sprawy o uchylenie i stwierdzenie wykonalności wyroku sądu polubownego — przez skill `law-pl-searching-orzeczenia` (sn.pl).
...
- **Podstawy odmowy (art. 1214 § 3 KPC + art. 1215 § 2 KPC)** — zasadniczo odpowiadają art. V NYC; zob. skill `law-pl-applying-new-york-convention`.
- **Termin:** brak terminu zawitego w KPC dla wniosku o uznanie / stwierdzenie wykonalności (odmiennie od niektórych jurysdykcji). Ograniczenie wynika z przedawnienia roszczenia zasądzonego — zob. skill `law-pl-checking-przedawnienie` (art. 125 KC — 6 lat od prawomocności).
...
7. **Dokumenty procesowe.** Request for Arbitration / pozew, Statement of Defence, wnioski. Zawsze zgodnie z regulaminem instytucji (przez skill `law-pl-fetching-arbitration-rules`).
8. **Wyrok → uznanie / wykonanie.** Dla wyroku z Polski — stwierdzenie wykonalności (art. 1213 KPC). Dla zagranicznego — uznanie + stwierdzenie wykonalności wg art. 1215 KPC + NYC (skill `law-pl-applying-new-york-convention`).
...
- **Regulaminy instytucji** — wyłącznie przez skill `law-pl-fetching-arbitration-rules` (WebFetch oficjalnych stron).
- **NYC 1958** — wyłącznie przez skill `law-pl-applying-new-york-convention`.
- **Orzecznictwo SN** — sn.pl (skill `law-pl-searching-orzeczenia`).
...
- Na końcu — **Lista kontrolna dla prawnika**:
  ...
  - [ ] Regulamin instytucji zweryfikowany na datę postępowania (skill `law-pl-fetching-arbitration-rules`)
...
- **Nie wymyślać kasacji SN ani ICSID awards.** Wszystkie cytaty z orzeczeń — wyłącznie po weryfikacji przez skill `law-pl-searching-orzeczenia` albo italaw.com / icsid.worldbank.org. Sygnatury i daty — dosłowne.
- **Nie cytować regulaminów z pamięci.** Każda wzmianka o Art. X of the ICC Rules / SCC Rules / SAKIG Regulaminu — wyłącznie po WebFetch przez skill `law-pl-fetching-arbitration-rules`.
